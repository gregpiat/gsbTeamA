...
function connectComptable($idIdentifiant){
    ...
    $ligne = false;     
    if ( $refComtable ) {
        $ligne = mysql_fetch_assoc($refComtable);
        $comptableReferent = $ligne["comptableReferent"];
        mysql_free_result($refComtable);
    }        
	return $comptableReferent;
}
...
function recupModifVisiteur($idVisiteur){
    $requete = "select * from visiteur where comptableReferent != '' and id='".$idVisiteur."'";
...
function UpdateModifVisiteur($idSaisi,$txtNom, $txtPrenom, $txtLogin, $txtMdp, $txtAdresse, $txtCp, $txtVille){
	$requete = "update visiteur set nom = '".$txtNom."', prenom = '".$txtPrenom."', login = '".$txtLogin."', mdp = '".$txtMdp."', adresse = '".$txtAdresse."', cp = '".$txtCp."', ville = '".$txtVille."' where id='".$idVisiteur."'";
	mysql_query($requete);
}
...
function obtenirDetailVisiteur($idCnx, $unId) {
    ...
    $requete = "select id, nom, prenom, comptableReferent from visiteur where id='" . $unId . "'";
